"""Create features for training POS CRF model from conll or ssf or ilci format annotated data."""
...
# input is the folder containing file in conll or ssf or ilci
...
def read_file_and_find_features_from_sentences(file_path, data_type='conll'):
    ...
    if data_type == 'conll':
        ...
        if search('ID\tValue', lines[0], IGNORECASE):
            lines = lines[1:]
...
def find_features_from_sentences(sentences, data_type='conll'):
    ...
    :data_type: Data type of the sentences - conll or ilci
    ...
    for sentence in sentences:
        sentence_features = ''
        if data_type in ['conll', 'ssf']:
            delimiter_token = '\n'
            delimiter_tag = '\t'
        else:
            delimiter_token = ' '
            delimiter_tag = '\\'
        for line in sentence.split(delimiter_token):
            if line:
                line_split = line.split(delimiter_tag)
                line_split = [item.strip() for item in line_split if item.strip()]
                if data_type in ['conll', 'ilci']:
                    token = line_split[0]
                else:
                    token = line_split[1]
                tag = line_split[-1]
...
def main():
    ...
    parser.add_argument('--type', dest='type', help="Add the type of the data either conll or ilci or ssf")
